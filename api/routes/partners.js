...
async function sendMemberDM(userId, embed) {
    try {
        if (!BOT_TOKEN) return;
        const dmChannel = await discordFetch(`https://discord.com/api/v10/users/${userId}/channels`, BOT_TOKEN, 'Bot ');
...
router.post('/api/partners/request', authMiddleware, async (req, res) => {
    try {
        ...
        try {
            const dmChannel = await discordFetch(`https://discord.com/api/v10/users/${req.user.id}/channels`, BOT_TOKEN, 'Bot ');
...
router.post('/api/admin/partner/requests/:id/approve', checkAdmin, async (req, res) => {
    try {
        ...
        try {
            const dmChannel = await discordFetch(`https://discord.com/api/v10/users/${partnerReq.user_id}/channels`, BOT_TOKEN, 'Bot ');
